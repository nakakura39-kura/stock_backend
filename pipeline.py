...
class MultiTimeframePatternEngine:
    ...
    # 네이버 1분봉 데이터를 15분/60분봉으로 리샘플링
    def _fetch_naver_intraday(self, code: str, timeframe_minutes: int) -> pd.DataFrame:
        # 최근 약 600개의 1분봉 수집
        df_1m = self._fetch_naver_chart(code, timeframe="1", count=600)
        if df_1m.empty:
            return pd.DataFrame()
        ...
        return self._clean_df(resampled)
    ...
    def fetch_intraday(self, code: str, is_us: bool, interval: str = "60m") -> pd.DataFrame:
        symbol = code.upper().strip()
        minutes = 60 if interval == "60m" else 15

        # 1. 한국 주식: 네이버 1분봉 크롤링 후 리샘플링 시도
        if not is_us:
            df_naver_resampled = self._fetch_naver_intraday(symbol, minutes)
            if not df_naver_resampled.empty and len(df_naver_resampled) >= 10:
                return df_naver_resampled
